...
import { NoticPayPreview } from "./NoticPayPreview";
...
// The three built/working experiences - each opens the exact same
// fullscreen NOTIC OS window "Open Experience" and the OS Projects app both
// read from.
const FEATURED_SLUGS = ["bmw", "noir", "notic-pay"];

/**
 * Real work, moved up and given room. Every entry here is the same
 * `osProjects` data NOTIC OS's own Projects window reads - "Open
 * Experience" opens the exact same fullscreen BMW/NOIR/NOTIC PAY window,
 * not a separate homepage-only preview.
 */
  ...
  return (
    <Scene id="work" tone="#0a0c10" fullHeight={false} className="py-section">
      <div className="mx-auto flex w-full max-w-content flex-col gap-2 pb-6">
        ...
        <h2 className="max-w-xl font-display text-h1 font-bold text-foreground">Three built experiences.</h2>
      </div>
      ...
      <div className="mx-auto flex w-full max-w-content flex-col">
        {projects.map((project, i) => (
          <WorkCard
            key={project.slug}
            project={project}
            image={PREVIEW_IMAGES[project.slug]}
            previewNode={project.slug === "notic-pay" ? <NoticPayPreview /> : undefined}
            reversed={i % 2 === 1}
          />
        ))}
      </div>
